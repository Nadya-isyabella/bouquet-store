...
<style>
    ...
    .badge-status.bg-success { background: #7fc29b !important; color: #1f4a38; }
    .badge-status.bg-warning { background: #f7c948 !important; color: #5a3e3e; }
    .badge-status.bg-danger { background: #e6717a !important; color: #5a2a2a; }
    .badge-status.bg-secondary { background: #b59595 !important; color: #3d2b2b; }
</style>
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <select name="status" id="status" class="form-select" required>
                        @php
                            $statusList = ['Aktif', 'Cuti', 'Sakit', 'Nonaktif'];
                            $statusColors = [
                                'Aktif' => 'bg-success',
                                'Cuti' => 'bg-warning',
                                'Sakit' => 'bg-danger',
                        ...
                        @foreach($statusList as $status)
                            <option value="{{ $status }}"
                                {{ old('status', $petugas->status) == $status ? 'selected' : '' }}>
                                {{ $status }}
                            </option>
                        @endforeach
                    </select>
                    <small class="text-muted">
                        Status saat ini:
                        <span class="badge-status {{ $statusColors[$petugas->status] ?? 'bg-secondary' }}">
                            {{ $petugas->status }}
                        </span>
                    </small>
                </div>
